...
print(parler_auth.is_logged_in)
...
def get_posts(username: str):
    ...
    with open(filename % (username, str(datetime.date.today()), "posts"), mode="a", encoding="utf-8") as posts:
        exputils.writetocsv(posts, [x.get("primary") for x in data.get("data").get("posts")], insert_headers=True)
    ...
    while True:
        ...
        try:
            ...
            if "code" in data and data.get("code") == "NO_DATA":
                ...
                break
            with open(filename % (username, str(datetime.date.today()), "posts"), mode="a", encoding="utf-8") as posts:
                exputils.writetocsv(posts, [x.get("primary") for x in data.get("data").get("posts")], insert_headers=False)
